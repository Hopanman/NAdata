...
#장르 갯수로 그룹 나누기
unique(mydf$GENRE_NM_NUM) #1~6

group1<-subset(mydf,GENRE_NM_NUM==1)
group2<-subset(mydf,GENRE_NM_NUM==2)
group3<-subset(mydf,GENRE_NM_NUM==3)
group4<-subset(mydf,GENRE_NM_NUM==4)
group5<-subset(mydf,GENRE_NM_NUM==5)
group6<-subset(mydf,GENRE_NM_NUM==6)
...
boxplot(y~group,
        main='Boxplot of 전국관객수 by 장르 갯수',
        xlab='Factor Levels:장르 갯수 1/2/3/4/5/6',
        ylab='전국관객수')
